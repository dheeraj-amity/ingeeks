...
const ADMIN_HASH = process.env.ADMIN_PASSWORD_HASH || ''; // must be provided via env
  ...
  providers: [
    Credentials({
      ...
      async authorize(credentials) {
        ...
        const ok = ADMIN_HASH && ADMIN_HASH.startsWith('$2') ? await bcrypt.compare(password, ADMIN_HASH) : false;
        if(!ok) return null;
